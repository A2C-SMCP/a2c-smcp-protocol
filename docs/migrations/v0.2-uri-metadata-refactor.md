...
3. **DPE 内容投递重构**：DPE 内容**不再走 Socket.IO**——Computer 端业务层注册 DPE Resolver hook，把 DPE Resource 转成 Agent 可访问的 URI（对象存储 / 本地文件 / 任意 scheme），Agent 用应用层协议（HTTP / file / ...）自取
4. **协议版本握手**：引入 Socket.IO connect 阶段的版本校验机制（详见 [versioning.md](../specification/versioning.md)）
...
4. **新增 `client:open_dpe`**：Agent 把 DPE URI 提交给 Computer，Computer 调本地业务 Resolver hook 转成访问 URI 返回；Resolver 实现完全交给业务方（上传 OSS / 落本地缓存 / 任意 scheme），协议不规定生命周期/可用性
5. **新增 Socket.IO 连接必带 URL query `?a2c_version=0.2.0`**；Server 在 HTTP 中间件层校验，不兼容时返回 HTTP 400 + `4008`
6. 元数据字段分工原则：MCP 标准 `annotations`（`priority` / `audience` / `lastModified`）**MUST** 放入 `annotations`；A2C 自定义字段（`fullscreen` / DPE 业务字段）放入 `_meta`。Window `audience=["assistant"]`、DPE `audience=["user"]`
...
| **MCP Manager** | `a2c_smcp/computer/mcp_clients/manager.py` | rust-sdk 决定 | 新增 host 唯一性校验与拒绝注册逻辑 |
| **Socket.IO Client（Agent 侧）**| `a2c_smcp/agent/socketio/client.py` | rust-sdk 决定 | 新增 `open_dpe` 方法；移除 `list_finder` |
| **Socket.IO Client（Computer 侧）**| `a2c_smcp/computer/socketio/client.py` | rust-sdk 决定 | 新增 `on_open_dpe` 处理器；移除 `on_list_finder` / `update_finder` |
| **数据类型** | `a2c_smcp/smcp.py` | rust-sdk 决定 | 新增 `OpenDPEReq` / `OpenDPERet`；删除 `ListFinderReq` / `ListFinderRet` / `DPEDocumentSummary` / `DPEPageSummary` / `DPEElementDetail` |
...
| **测试** | `tests/unit_tests/computer/`、`tests/integration_tests/` | rust-sdk 决定 | 所有涉及 URI query 解析、Finder 事件、`keywords` 参数的测试要重写或删除；新增 `open_dpe` / Resolver / 版本握手矩阵测试 |
...
| B3 | DPE URI query 移除 | `dpe://host/doc?format=md&offset=0` | `dpe://host/doc-ref[/sub-path]` | **硬切换**；DPE URI 纯标识符化，content-control query（`format` / `depth` / `offset` / `limit` / `categories`）全部移除——内容形态由业务 Resolver 决定 |
...
| **B6** | **新增 `client:open_dpe`** | 无（v0.1 通过 list_finder + MCP `resources/read` 直接返回内容）| `client:open_dpe(uri) → OpenDPERet(uri=访问URI)`；DPE 内容**不走 Socket.IO**，由业务 Resolver hook 转成访问 URI（HTTP/file/任意 scheme），Agent 应用层自取 | **新增**；SDK 必须实现该事件；Computer 必须暴露 Resolver 注册 API |
| B7 | host 唯一性 | SHOULD（v0.1 后期加） | **MUST** 全局唯一；同一 Computer 进程内跨 Office/Room 唯一；冲突时 Computer MUST 拒绝后注册者 | 新增约束，旧实现若无冲突场景不受影响 |
...
| N2 | DPE 资源访问错误码 | 无 | `4011 DPE Resolver Not Configured` / `4012 Invalid DPE URI` / `4013 DPE Resolution Failed` | 新增；为 `client:open_dpe` 路径提供专属错误语义 |
...
- `host` 在同一 Computer 进程范围内 **MUST** 全局唯一
...
// 新（纯标识符，不携带任何 query）
dpe://host/doc-ref[/pages/N | /elements/ID]
```

#### 解析器行为

- **拒绝 `dpe://host` 形式**（无 doc-ref）；返回解析失败 / `4012`
- **拒绝 query 参数**：解析时若检测到 query，记录 WARN 并丢弃；Agent SDK 构造 URI 时**不应**附加 query
- `host` 在同一 Computer 进程范围内 **MUST** 全局唯一（与 `window://` 共享 host 命名空间约束）
...
### 4.3 client:open_dpe 与 DPE Resolver Hook（新增）
...
```
Agent → client:open_dpe(uri) → Computer → DPE Resolver Hook → 访问 URI
                                              │
                                              ▼
                                     业务存储（OSS / 本地 / etc）
                                              │
...
| 事件常量 | 事件名称 | 数据结构 |
|---|---|---|
| `OPEN_DPE_EVENT` | `client:open_dpe` | `OpenDPEReq` → `OpenDPERet` |
...
未注册时收到 `client:open_dpe` 必须返回 `4011 DPE Resolver Not Configured`——**禁止**降级到 inline 透传 ResourceContents。

#### Computer 处理流程

1. 接收 `client:open_dpe(uri=dpe://...)`
2. 校验 URI（dpe scheme + 非空 host + 非空 doc-ref + sub-path 合法）；非法返回 `4012`
...
`window://` 与 `dpe://` 共享 host 命名空间——同一 Computer 进程范围内 host **MUST** 全局唯一（跨 Office/Room 也唯一）。两个 MCP Server 声明相同 host 时，Computer **MUST** 拒绝后注册者。

**作用域生命周期**：

- host 索引仅存活于 Computer 进程生命周期内，进程重启后清空重建
- Computer 切换 office 时**不**重置 host 索引（host 注册关系与 office 解耦）
- MCP Server 主动断开时 host 占用解除，可被新 Server 复用

详见 [Window URI 校验规则](../specification/desktop.md#window-uri-规范) / [DPE URI 校验规则](../specification/dpe.md#dpe-uri-规范)。
...
HTTP 中间件层校验 `a2c_version`，不兼容返回 HTTP 400 + `4008`。详细规范、Python/Rust 实现示例、跨语言通用约束见 [versioning.md](../specification/versioning.md)。
...
3. **DPE 内容形态自决**：A2C 协议**不规定** `resources/read` 响应 JSON schema；与 Agent / Resolver 之间约定即可
...
6. **新增 `client:open_dpe` 处理器**：按 [§4.3 处理流程](#computer-处理流程) 实现
7. **MCP Manager**：注册时校验 host 唯一性（`window://` 与 `dpe://` 共享）；冲突拒绝后注册者
8. **错误码枚举**：新增 4006 / 4007 / 4008 / 4011 / 4012 / 4013

### 5.3 Agent SDK 工程师

1. **删除 `list_finder` 方法**
2. **新增 `open_dpe(uri)` 方法**：返回 `OpenDPERet`；Agent 业务代码拿到访问 URI 后用应用层协议自取实际内容
3. **HTTP / file URI 内置拉取**：建议 SDK 内置 http(s):// 与 file:// 的拉取能力；其他 scheme 由用户业务代码处理
4. **不缓存 URI 跨 session**：拿到 URI 仅在本次会话使用；过期时重新调用 `open_dpe`
5. **协议版本握手**：连接 URL query 拼 `?a2c_version=...`；解析 HTTP 400 body 识别 4008 → 主动 `disconnect()` → 抛 `ProtocolVersionError`
...
### 6.3 client:open_dpe + Resolver

| 用例 | 预期 |
|---|---|
| 注册 Resolver + 合法 dpe URI | ✅ 返回 `OpenDPERet(uri=...)` |
| 未注册 Resolver | ❌ `4011 DPE Resolver Not Configured` |
| 非法 URI（缺 doc-ref / 非 dpe scheme / 非法 sub-path / 含 query）| ❌ `4012 Invalid DPE URI` |
...
| 用例 | 预期 |
|---|---|
| 两个 MCP Server 声明相同 host | ❌ 后注册者被拒绝 |
| 同一 host 跨 Office/Room | ❌ 仍冲突（host 索引与 office 解耦）|
| Computer 重启后重新注册 | ✅ host 索引清空，可重新注册 |
...
- 完整 `open_dpe` 流程：Mock MCP Server 返回 `dpe://` Resource → Agent open_dpe → Computer 调 Resolver → Agent 用应用层拉取
...
v0.1.x 与 v0.2.0 在 URI 形态、Finder 移除、`open_dpe` 新增等多处**无法互通**。Server 端通过 HTTP 中间件 `a2c_version` 校验，不兼容直接拒绝（HTTP 400 + `4008`）。
...
```
阶段 1：发布 v0.2 Computer SDK（含 DPE Resolver API + open_dpe）
...
阶段 5：发布 v0.2 Agent SDK（含 open_dpe 客户端 + 应用层拉取）
...
- `docs/specification/dpe.md`：**新建**（替代旧 finder.md）
...
- `docs/specification/data-structures.md`：删除 `ListFinderReq` / `ListFinderRet` / `DPEDocumentSummary` / `DPEPageSummary` / `DPEElementDetail`；新增 `OpenDPEReq` / `OpenDPERet`
- `docs/specification/events.md`：删除 `client:list_finder` / `server:update_finder` / `notify:update_finder`；新增 `client:open_dpe`
