...
4. **新增 `client:get_dpe`**：Agent 把 DPE URI 提交给 Computer，Computer 调本地业务 Resolver hook 转成访问 URI 返回；Resolver 实现完全交给业务方（上传 OSS / 落本地缓存 / 任意 scheme），协议不规定生命周期/可用性
4'. **新增 `client:get_resources`**：透明转发 MCP 标准 `resources/list` 给 Agent（含 cursor 翻页）；`mcp_server` 必填，Computer 不做协议级过滤；与 `client:get_config` 组合实现 Agent 端的 dpe / window / 业务自定义资源发现，替代 v0.1 Finder 聚合视图
...
6. **新增 `client:get_dpe` 处理器**：按 [§4.3 处理流程](#computer-处理流程) 实现
...
| host 重复时 `get_dpe(uri=dpe://repeated-host/...)` | ⚠️ 路由到**先注册的** Server + 记 WARN；后注册者的同 URI 资源无法被命中 |
