...
### name 与 bundle_id：声明面与身份面

上例中 `servers` 的 map key（及 `name` 字段）**不是 server 的运行期身份**——它是**声明面**的人写名字。两者是正交的命名空间：

| | `servers` map key / `name` | `bundle_id` |
|---|---|---|
| 谁写谁读 | **人写、人读**（配置撰写、展示） | **代码写、代码读**（运行期传递与标识） |
| 唯一性 | **允许碰撞**、永不做键/寻址、不强制唯一（[身份正交性](../specification/data-structures.md#identity-orthogonality)） | 运行期唯一身份：`get_config.servers` 的 key、工具暴露名前缀、错误码 `meta.mcp_server`、env 变量 server 段全用它 |
| 来源 | 用户手写 | **derive-on-load 派生**（见下），或显式声明 `bundleId` 字段；派生结果 **MUST NOT 回写配置源** |

**缺省派生规则**：`bundle_id = 显式 bundleId ?? normalize(name)`；`normalize(name)` 为空（如名字全为非 ASCII）时回落 `bundle_` + 连接身份摘要。绝大多数情况下 `bundle_id` 就是规范化后的 name，稳定且不依赖 command/args。

**何时需要显式写 `bundleId`**：

1. 两个 key 归一到同一 bundle_id（如 `my.server` 与 `my_server` 都归一为 `my_server`）——这是**注册期硬错误**（fail-fast），解法即改名或为其一显式指定 `bundleId`；
2. env 变量名坍缩 fail-fast（见 [环境变量命名规则](#环境变量命名规则双端统一规范)）；
3. 名字全为非 ASCII 又希望身份可读、可预期时。

**工具暴露名**：聚合后进入 Agent 工具列表的名字是 `{bundle_id}__{alias ?? 原始工具名}`（[BundleID 模型](../specification/data-structures.md#mcp-tool-命名与路由)），跨 server 同名工具因此天然不冲突；每个工具的 server 归属由 `SMCPTool.bundle_id` 字段显式携带。
...
环境变量提供者使用以下命名规则查找对应的值（**本节为双 SDK 通用规范，双端 MUST 逐字节一致**）：

```
{前缀}ENV_SEGMENT(INPUT_ID)[_ENV_SEGMENT(BUNDLE_ID)][_ENV_SEGMENT(TOOL_NAME)]
```

- **前缀统一为 `A2C_SMCP_`**（0.3.0 起；历史 `A2C_INPUT_` 前缀废止，直接硬切、无过渡期）
- server 上下文段使用该 server 的 **`bundle_id`**（运行期唯一身份），**不使用 display name**
- **`ENV_SEGMENT` 规范化函数（全段统一同一个函数）**：**保留大小写** + 将 `[^A-Za-z0-9_]` 逐字符替换为 `_`
    - 保留大小写：POSIX env 名本就大小写敏感；折叠大小写会让 `MyServer` / `myserver`（两个合法且不同的 bundle_id）坍缩到同一变量名
    - 字符替换：覆盖 bundle_id 中的 `-`，也覆盖 INPUT_ID 中可能出现的 `@`、`/`（带前缀池条目如 `frontend@team/figma_token`）
- **坍缩 fail-fast**：`ENV_SEGMENT` 非单射（如 bundle_id `a-b` 与 `a_b` 均映射为 `a_b`）。SDK MUST 在注册期计算全部活跃 env 变量名，检测到**完整变量名**相同（不同身份映射到同一变量名）即硬错误，提示用户为其一显式指定 `bundleId`——与「同文件多 key 归一同 bundle_id fail-fast」同款模式。仅 `ENV_SEGMENT` 相同但 input id 不同的情形无害，不报错

**示例**：

| Input ID | Server 上下文（bundle_id） | 环境变量名 |
|----------|--------------|-----------|
| `api_key` | 无 | `A2C_SMCP_api_key` |
| `api_key` | `feishu-mcp` | `A2C_SMCP_api_key_feishu_mcp` |
| `token` | `MyServer` + tool `auth` | `A2C_SMCP_token_MyServer_auth` |
| `frontend@team/figma_token`（带前缀池条目） | 无 | `A2C_SMCP_frontend_team_figma_token` |

环境变量会根据 Input 类型自动进行类型转换：
...
post-BundleID，**跨 Server 的工具同名本身已不构成冲突**——工具暴露名恒为 `{bundle_id}__{alias ?? 原始名}`，`bundle_id` 前缀在构造上保证跨 Server 唯一。`alias` 的作用是让暴露名的**工具段**更具业务语义（也可在暴露名上区分同一 server 的不同用途封装）：
...
Agent 端看到的工具名是 `web-search__web_search` 和 `doc-search__doc_search`（`{bundle_id}__{alias}`；工具的 server 归属另由 `SMCPTool.bundle_id` 字段显式携带）。调用时 SDK 按暴露名整键查表路由回正确 MCP Server 的原始工具名。

!!! warning "`default_tool_meta.alias` 是完整替换语义"

    `alias` 写在 `default_tool_meta` 里会回落到该 server 的**每一个**未单独配置的工具——多个工具将拿到**同一个 alias**、撞出相同暴露名，除第一个外全部静默不可调用。`alias` 只应写在 `tool_meta.<具体工具>` 下。
...
| **环境变量前缀** | `A2C_SMCP_`（双端统一规范，见 [环境变量命名规则](#环境变量命名规则双端统一规范)） | 同左（`with_prefix()` 仅限测试场景，生产 MUST 保持默认） |
