...
!!! danger "v0.2 重大变更（破坏性）"

    自协议版本 **0.2** 起，Finder 有以下不向后兼容的变更，SDK 实现需同步升级：

    1. **Level 0 URI 移除**：`dpe://host` 形式无效；文档发现改走 MCP `resources/list`
    2. **文档元数据下沉到 Resource**：`keywords` / `file_type` / `page_count` / `file_uri` 改为 `Resource._meta.*` 声明；`last_modified` 改为 `Resource.annotations.lastModified`
    3. **Finder 事件重命名**：`client:get_finder` → `client:list_finder`；类型 `GetFinderReq/Ret` → `ListFinderReq/Ret`、常量 `GET_FINDER_EVENT` → `LIST_FINDER_EVENT`、Agent SDK 方法 `agent.get_finder()` → `agent.list_finder()`（对齐 REST "集合查询 + 分页" 语义；`get_desktop` 不变）
    4. **`ListFinderReq.keywords` 参数重命名为 `tags`**（语义不变：对 `title` / `keywords` / `summary` 做 fuzzy 命中）
    5. **Window URI query 参数移除**：`window://...?priority=P&fullscreen=F` 改为 `Resource.annotations.priority`（float [0,1]）+ `Resource._meta.fullscreen`（见 `desktop.md`）

    本指南**部分代码示例（如 filter 函数实现、MCP Server read_resource 示例）仍基于旧模型**，规范性定义请以 `specification/finder.md` 为准。下列结构定义已同步更新，可直接参考。
...
└──────────────┼─────────────────────────────────────┘
               ...
               ▼
          ┌─────────┐
          │  Agent   │ ← client:list_finder + MCP resources/read
          └─────────┘
```
...
| 角色 | 职责 | 工作量 |
|------|------|--------|
| **Server** | 纯路由 — 转发 `client:list_finder`、广播 `notify:update_finder` | 最小 |
...
| `GET_DESKTOP_EVENT` | `LIST_FINDER_EVENT` | 相同路由机制 |
...
| `GetDeskTopReq/Ret` | `ListFinderReq/Ret` | Finder 多了 `keywords`、`file_type`、分页 |
...
=== "Python"
    ...
    ```python
    LIST_FINDER_EVENT = "client:list_finder"
...
=== "Rust"
    ...
    ```rust
    pub const LIST_FINDER_EVENT: &str = "client:list_finder";
...
=== "Python"
    ...
    class ListFinderReq(AgentCallData, total=True):
        ...
        tags: NotRequired[list[str]]        # 可选：标签过滤；每个 tag 在文档 title/keywords/summary 中 fuzzy 命中
...
=== "Python"
    ...
    @self.on(LIST_FINDER_EVENT)
    async def handle_list_finder(sid, data: ListFinderReq):
...
=== "Rust"
    ...
    async fn handle_list_finder(
        &self,
        sid: Sid,
        data: ListFinderReq,
    ) -> Result<ListFinderRet, SmcpError> {
        ...
        self.emit(LIST_FINDER_EVENT, &data, *computer_sid).await
    }
    ```
...
=== "Rust"
    ...
    ```rust
    async fn handle_list_finder(&self, data: ListFinderReq) -> Result<ListFinderRet> {
        // 1. 收集文档
...
=== "Rust"
    ...
    pub async fn list_finder(
        ...
        offset: Option<i64>,
        limit: Option<i64>,
    ) -> Result<ListFinderRet> {
        let req = ListFinderReq {
        ...
        self.sio.emit(LIST_FINDER_EVENT, &req).await
    }
    ```

### 通知处理器

注册 `notify:update_finder` 监听器，推荐收到通知后自动调用 `list_finder()` 刷新文档目录。
...
=== "Python"
    ...
    async def on_finder_updated(data: UpdateComputerConfigReq):
        ...
        # 推荐：自动刷新文档目录
        new_finder = await self.list_finder(computer=computer)
...
=== "Rust"
    ...
    self.sio.on(UPDATE_FINDER_NOTIFICATION, |data: UpdateComputerConfigReq| {
        ...
        // 推荐：自动刷新文档目录
        let new_finder = self.list_finder(&computer, None, None, None, None).await?;
...
- 10 条校验规则覆盖（每条规则至少一个正例 + 一个反例）
- 三级 URI 解析（Level 1 ~ Level 3；Level 0 已废弃，`dpe://host` 形式应被视为无效）
...
- Tag 过滤 → 对 `title`（`Resource.name`）、`_meta.keywords`、`description`（summary） 分别做 fuzzy 命中
- 文件类型过滤（`_meta.file_type`）→ 精确匹配、不匹配时返回空
- Server 优先级 → 最近使用的 Server 排在前面
- `annotations.lastModified` 排序 → 降序、缺失值排末尾
- 分页边界 → `offset` 超出范围、`limit = 0`、`limit > total`
- `total_count` → 反映过滤后的总数，而非分页后的数量
- 元数据合成 → 从 `Resource` + `annotations` + `_meta` 合成 `DPEDocumentSummary`，`_meta.page_count` 缺失时跳过并告警
...
- Agent 发送 `client:list_finder` → 收到正确的 `ListFinderRet`
- 带 `tags` / `file_type` / `offset` / `limit` 参数的过滤请求
...
- `client:list_finder` → Level 1 → Level 2 → Level 3 逐级读取
- 带 content-control 参数的读取（`depth=pages`、`format=markdown`、`categories=table`）
...
- [ ] **注册 `client:list_finder` 事件处理器** — 路由转发（小）
...
- [ ] **`client:list_finder` 事件处理器** — 收集 → 组织 → 返回（小）
