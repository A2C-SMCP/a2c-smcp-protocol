...
协议版本放在 **URL query string** 中传输，而不是 `auth` dict 或自定义 HTTP header。三方案权衡：
...
1. **校验时机硬约束**——版本是"能不能说同一种话"（协议层），认证是"你是谁"（业务层）。`auth` 对象是 Socket.IO 为**应用层身份认证**提供的入口，其处理发生在用户编写的 `connect` handler 中；handler 异常、中间件拦截、auth 字段被改写等情况都可能让版本校验失效。**协议层校验必须先于业务层**——这要求载体在 HTTP 层可见
2. **SDK 一致性**——URL query 是 socketio 客户端最一致的 HTTP 层透传方式（所有 SDK 在 `connect()` API 都原生支持 query）；HTTP header 透传 API 各 SDK 不齐整（python-socketio 用 `headers=` 参数；某些 TS SDK 需要 `extraHeaders` 配置）
3. **`a2c_version` 不是敏感信息**——版本号出现在 access log 无安全风险；"URL query 出现在 access log" 的反对意见在版本号语义下不成立
4. **同时支持两种载体 = 协议复杂度膨胀**——Server 必须 fallback 检查、SDK 必须双路径解析、版本演进时调和两种载体的语义会再爆雷

因此协议版本放在 **URL query string** 中，校验在 HTTP 层完成；身份相关字段（如 `role`）留在 `auth` 对象。两者职责清晰分离。
...
## 开发中版本与版本冻结（Unreleased Lifecycle）

**版本号只在发布那一刻冻结。** 未发布的版本是一个开放收集桶：发布前进入的所有变更（含破坏性）都折进该版本，版本号不再往上抬。「要不要 bump」只对**已发布**版本之后的新变更成立。

### 唯一权威标记：`-dev` 后缀

开发中的版本序列化为 `X.Y.Z-dev`（如 `0.3.0-dev`），发布时去掉后缀落到 `X.Y.Z`。该后缀是「未发布」的唯一权威、可机读标记，MUST 同时出现在版本单一来源（`pyproject.toml`）与文档版本字段。两站 `latest` 恒指向已**发布**（无 `-dev`）的最高版本。

### 判定程序（决定是否 bump 前必做）

1. 读当前版本串。
2. **带 `-dev`（未发布）** → 新变更折进当前版本，**不 bump、不新起号、不另发布**；仅当变更严重度超过当前目标位才就地抬高目标位（如 `0.3.0-dev` → `1.0.0-dev`）。
3. **无 `-dev`（已发布）** → 才按上文 [版本号语义](#版本号语义majorminorpatch) 决定新目标位，开启新的 `-dev` 周期。

### 与 bump 工具的映射

| 动作 | 命令 | 效果 |
|---|---|---|
| 开周期 | `bump-my-version bump {major\|minor\|patch}` | 低位归零、`pre_l` 归 `dev`，得 `X.Y.Z-dev` |
| 发布 | `bump-my-version bump pre_l` | `dev` → `final`，去掉后缀得 `X.Y.Z`（并打 tag `vX.Y.Z`、推 `latest`） |
...
> **注**：协议规范仅提供 Python reference impl。其他 SDK（Rust / TypeScript / Go 等）的连接示例由各 SDK 自行决定——A2C-SMCP 协议文档不堆砌多语言示例。

!!! warning "polling-first 是 MUST（v0.2.1 由 SHOULD 收紧）"

    客户端 **MUST** 令**首个**握手走 HTTP polling（`transports` 以 `polling` 起始或等效配置）。理由：版本校验依赖一个可承载 HTTP 400 + 4008 body 的响应阶段；直连 WebSocket 在 ASGI 下是 `websocket` scope、无常规 HTTP 响应阶段。SDK **MUST NOT** 默认 WebSocket-only。若调用方**显式**强制 WS-only，握手不被豁免——版本拒绝退化为 [§5](#5-websocket-only-握手的版本拒绝形态) 定义的 WS 形态（服务端仍强制校验，保证传递性对**任意**客户端无条件成立）。
...
| 约束 | 说明 |
|------|------|
| **MUST** 在 Engine.IO 握手到达 Socket.IO 业务代码之前完成 | 确保 `connect` handler 即使有 bug 也无法绕过版本校验 |
| **MUST** 同时校验 polling 握手与直连 WebSocket 握手两条建连路径 | 二者 `a2c_version` 均在 query string 中；ASGI 下分别是 `http` 与 `websocket` scope。**只校验 `http` scope 会让 `transports=["websocket"]` 客户端绕过版本闸门、击穿传递性保证**。WS-only 握手的校验与拒绝形态见 [§5](#5-websocket-only-握手的版本拒绝形态) |
            ...
            return JSONResponse(
                {
                    ...
                    "min_supported": str(SERVER_MIN_SUPPORTED),
                    "max_supported": str(SERVER_MAX_SUPPORTED),
                },
                status_code=400,
                headers={"X-A2C-Error-Code": "4008"},
            )
...
!!! warning "4008 是 HTTP body code，不是 WS close code"
    ...
    polling 握手路径上版本校验在 HTTP 层完成、发生在 WS 帧建立之前。**4008 是 ErrorPayload.code 字段值，承载于 HTTP 400 响应 body 中**——SDK 实现 **MUST NOT** 把 4008 与 WebSocket close code 混淆。WS-only 直连握手另有拒绝形态（首选 denial-response 仍回**字节一致**的 4008 body；栈不支持时回退到协议指定的、与 `4008` **取不同值**的 WS close code），见 [§5](#5-websocket-only-握手的版本拒绝形态)。完整的 4008 解析模式见 [error-handling.md §协议版本不匹配（4008）](error-handling.md#协议版本不匹配4008)。

Socket.IO 客户端遇到 HTTP 400 响应会触发 `connect_error` 事件，或在 `connect()` 调用时抛 `ConnectionError`。SDK **必须**解析响应 body，识别 `code: 4008` 并抛出专属异常。响应 body 不可访问的罕见 transport 边角情况下，可从 `X-A2C-Error-Code` 响应 header 中识别错误类型作为冗余诊断辅助。
...
try:
    ...
    if body.get("code") == 4008:
        raise ProtocolVersionError(
            ...
            message=body.get("message", "Protocol version mismatch"),
        )
    raise
```

> 协议规范仅提供 Python reference impl。其他 SDK 的具体解析模式由各 SDK 自行决定——A2C-SMCP 协议文档不堆砌多语言示例。
...
### 5. WebSocket-only 握手的版本拒绝形态

ASGI 把**直连 WebSocket**（客户端 `transports=["websocket"]`，跳过 polling）呈现为 `websocket` scope（**不是** `http`）。若服务端中间件只处理 `http` scope，这类客户端将**绕过版本闸门**——而 [§设计取向](#设计取向) 的传递性保证（"同房间成员必然兼容，无需 peer 二次校验"）依赖闸门对**任意**客户端无条件成立。本节定义该路径的强制行为，消除该缺口。

#### R1 客户端 polling-first（MUST）

见 [§1](#1-client-在-url-中声明版本)：conforming 客户端首个握手 MUST 走 HTTP polling，故正常路径**根本不会**进入本节。本节仅约束**非 conforming / 显式 WS-only** 客户端，使闸门对它们仍然有效。

#### R2 服务端 MUST 校验 websocket scope

服务端版本中间件 **MUST** 在 ASGI `websocket` scope（直连 WS 握手）上执行与 `http` scope **相同**的校验：`a2c_version` 同样位于 `scope["query_string"]`，判定逻辑复用 `is_compatible`，**不得**因 scope 类型不同而跳过。

#### R3 拒绝形态（二选一，按运行栈能力）

| 运行栈 | 服务端 MUST 行为 | 客户端可见 |
|---|---|---|
| 支持 ASGI **WebSocket Denial Response**（uvicorn ≥0.21 / hypercorn / daphne 等）| 在 WS 打开**之前**返回 **HTTP 400 + 与 polling 路径字节一致的 4008 flat ErrorPayload + `X-A2C-Error-Code: 4008`** | 与 polling 路径**完全一致**，4008 仍是单一事实源，SDK 复用同一解析路径 |
| 不支持 denial-response | 以协议指定的 WebSocket close code **`4900`** 关闭握手 | 收到 close code `4900` |
...
- **MUST NOT** 自动重连（同 [§4](#4-client-sdk-must-主动断开连接) 死循环防御）
- **SHOULD** 用 **polling-only** transport 重连一次，取回权威 HTTP 400 + 4008 body，再抛 `ProtocolVersionError`（拿到完整 `server_version`/`client_version`/`min_supported`/`max_supported`）
- 若重连仍拿不到 body：直接以 `4900` 抛 `ProtocolVersionError`，`message` 注明"WS-only 握手被服务端按版本不匹配拒绝；请改用 polling 重连以获取版本详情"，版本字段可为空

> **净效果**：传递性保证对**任意**客户端无条件成立——conforming 客户端走 polling 得标准 4008；非 conforming WS-only 客户端在支持 denial-response 的栈上同样得字节一致的 4008，否则得可判定的 `4900`。服务端在任何情形下都**不会**放行版本不兼容的连接。
...
- **响应结构**（HTTP 400 + `X-A2C-Error-Code: 4008` header + body）：
...
完整字段语义、SDK 解析示例（Python reference impl）见 [error-handling.md §协议版本不匹配（4008）](error-handling.md#协议版本不匹配4008)。Client SDK **必须**将此错误转换为明确异常，而不是静默重试。

- **承载层**：4008 始终是 **HTTP 400 body** 的 `ErrorPayload.code`。WS-only 直连握手在支持 denial-response 的运行栈上**仍回字节一致的 4008 HTTP body**；不支持时改用下面的 `4900`。

### 4900 WS Version-Handshake Rejected（WebSocket close code，非 ErrorPayload.code）

- **触发**：客户端违反 [§1 polling-first MUST](#1-client-在-url-中声明版本) 强制直连 WebSocket，且服务端运行栈不支持 ASGI WebSocket Denial Response
- **作用**：服务端按版本不匹配拒绝 WS 握手时关闭连接所用的 **WebSocket close code**（RFC 6455 私有段）
- **与 4008 的关系**：`4900` 与 `4008` **是不同命名空间的不同值**，**MUST NOT** 互相混淆或互换；`4900` 不携带 body，客户端按 [§5 R4](#5-websocket-only-握手的版本拒绝形态) 处理（SHOULD 改 polling 重连取回权威 4008）
- 完整语义见 [§5](#5-websocket-only-握手的版本拒绝形态)
...
SDK 包版本号与协议版本号的关系遵循以下规则：

| 维度 | 约束 |
|------|------|
| **PROTOCOL_VERSION 常量** | **MUST** 暴露——SDK 在代码中显式导出所实现的协议版本字符串。运行时握手以**该常量**为准（不是 SDK 包版本），握手层与包版本完全解耦 |
| **SDK MAJOR.MINOR vs 协议 MAJOR.MINOR** | **SHOULD** 对齐（推荐而非强制）——便于用户从包版本第一眼判断目标协议；SDK 自身可独立 SemVer 演进，不被协议 MINOR bump 强行绑住 |
| **SDK PATCH** | **自由**——留给 SDK 实现层的 bug 修复、性能优化、依赖升级 |
| **README / CHANGELOG 兼容矩阵** | SDK 仓库 **MUST** 维护一张"SDK 包版本 → 实现的协议版本"兼容矩阵；CHANGELOG 在影响协议字段映射时显式标注 |

**为什么松绑 MAJOR.MINOR 强约束**：

- 握手层（`a2c_version` URL query）传的是 SDK 暴露的 `PROTOCOL_VERSION` **常量**，不是 SDK 包版本——强绑定不必要
- 强绑定剥夺 SDK 自身 maturity 演进空间（如 rust-sdk 内部要做较大 API 改进，会被协议 MINOR 绑住跳号）
- MCP / LSP / JSON-RPC 等客户端库均无此惯例
- v0.x 早期生态小、迭代快，更应给 SDK 演进留弹性

**用户体验**：

- 用户安装 `pip install a2c-smcp==0.2.5` → SDK 暴露 `PROTOCOL_VERSION="0.2.0"` → 显式知晓目标协议
- 兼容矩阵在 README 一眼可见，无需翻代码

**示例升级路径**（首次实现 v0.2 时，SHOULD 但非 MUST）：

```
rust-sdk 0.1.15 (协议 0.1.x) → 0.2.0 (协议 0.2.0)         # 推荐 MAJOR.MINOR 对齐协议
rust-sdk 0.2.0 → 0.2.1 (协议仍 0.2.0)                     # SDK bug fix，PATCH 自由
rust-sdk 0.2.1 → 0.3.0 (协议仍 0.2.0)                     # SDK 自身 API 大改进，可独立跳 MINOR；
                                                           # README 兼容矩阵清楚标注：rust-sdk 0.3.x → 协议 0.2.x

python-sdk 0.1.5 (协议 0.1.x) → 0.2.0 (协议 0.2.0)        # 同上推荐
```

**PROTOCOL_VERSION 常量暴露**（MUST）：

```python
# python-sdk reference impl
from a2c_smcp import PROTOCOL_VERSION  # "0.2.0"
```

具体的常量暴露位置由各 SDK 自决（python-sdk 在 `a2c_smcp/__init__.py` / rust-sdk 在 crate 根模块 / TS SDK 在 package main）——协议规范不规定具体路径。

**MUST**：客户端 SDK 在连接时自动从 `PROTOCOL_VERSION` 常量读取 `a2c_version` 拼入 URL，**禁止**让用户代码手动传入（避免漂移）。

**SDK 兼容矩阵示例**（README 推荐格式）：

```markdown
| SDK 版本 | 实现的协议版本 |
|---------|----------------|
| 0.2.0 - 0.2.x | 0.2.x |
| 0.3.0 - 0.3.x | 0.2.x （SDK 内部重构，协议未变） |
| 0.4.0 - 0.4.x | 0.3.x |
```
...
| WS-only 兼容（直连 websocket） | 0.2.0 | 0.2.0 | ✅ 连接（`websocket` scope 同样被校验后放行） |
| WS-only 不兼容（栈支持 denial-response） | 0.3.0 | 0.2.0 | ❌ HTTP 400 / 4008（与 polling 路径字节一致） |
| WS-only 不兼容（栈无 denial-response） | 0.3.0 | 0.2.0 | ❌ WS close `4900`；client SHOULD 改 polling 重连取 4008 |
| 中间件仅校验 http、漏 websocket scope | 0.3.0 | 0.2.0（WS-only client） | ❌ 实现缺陷：连接被错误放行（回归测试 MUST 覆盖此反例） |
