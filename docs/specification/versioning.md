...
!!! warning "4008 是 HTTP body code，不是 WS close code"
    ...
    协议版本校验在 HTTP 层完成，发生在 WS 帧建立之前。**4008 是 ErrorPayload.code 字段值，承载于 HTTP 400 响应 body 中**——SDK 实现 **MUST NOT** 把 4008 与 WebSocket close code 4xxx 混淆。完整的 4008 解析模式见 [error-handling.md §协议版本不匹配（4008）](error-handling.md#协议版本不匹配4008)。
