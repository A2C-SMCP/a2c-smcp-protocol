...
| `host` | 是 | MCP Server 资源命名空间根 | 不能为空；**单个 MCP Server 内部的 URI 由 MCP `resources/list` 自身保证唯一**；跨 MCP Server **SHOULD** 唯一；推荐反向域名风格，如 `com.example.mcp` |
...
1. `scheme` 必须为 `window`
2. `host` 不能为空
3. `host` 跨 MCP Server **SHOULD** 唯一。Window URI 内部的 desktop 组织算法按 MCP Server 的 **`bundle_id`** 分组（server 唯一身份，避免同名 server 窗口误并；`bundle_id` 见 [数据结构 §身份正交性](data-structures.md#identity-orthogonality)），不强依赖 host 唯一；冲突时 Computer 在 MCP Server 注册阶段记 WARN（lint-style 引导，不阻塞注册）。host（MCP Server 自选）**SHOULD** 使用反向域名风格（如 `com.example.editor`）
...
1. **host 使用反向域名风格**（如 `com.example.myapp`）以提升可读性 + 天然回避跨 Server 冲突；跨 MCP Server **SHOULD** 唯一，冲突时 Computer 注册阶段记 WARN
