...
| `host` | 是 | MCP Server 资源命名空间根 | 不能为空；**单个 MCP Server 内部的 URI 由 MCP `resources/list` 自身保证唯一**；跨 MCP Server **SHOULD** 唯一（与 `dpe://` 共享 host 命名空间约束，详见 [DPE host 路由策略](dpe.md#host-路由策略)）；推荐反向域名风格，如 `com.example.mcp` |
...
1. `scheme` 必须为 `window`
2. `host` 不能为空
3. `host` 跨 MCP Server **MUST** 唯一（与 `dpe://` 共享 host 命名空间约束）。Window URI 内部的 desktop 组织算法本身按 MCP Server 名称分组、不强依赖 host 唯一；但 `client:get_dpe` 通过 host 反查 MCP Server，因此 Computer **MUST** 在 MCP Server 注册阶段检测 host 冲突——冲突时**注册失败**（硬约束）。详见 [DPE host 路由策略](dpe.md#host-路由策略) 与 [host 命名规范](dpe.md#host-命名规范)。host **SHOULD** 使用反向域名风格（如 `com.example.editor`，lint-style 引导，不阻塞注册）
...
MCP Server 通过 `resources/list` 返回的 `Resource` 对象上的 `annotations` 与 `_meta` 字段声明 Window 的布局与展示属性。Computer 在组织 Desktop 时直接读取这些字段，**不再解析 URI query**。

A2C v0.2 的元数据**字段分工原则**：语义与 MCP 标准 `annotations` 对齐的字段（`priority` / `audience` / `lastModified`）**MUST** 放入 `annotations`；A2C 自定义、无对应 MCP 标准的字段（`fullscreen` 等）放入 `_meta`。

### MCP 标准字段（`annotations`）

| 字段 | 类型 | 必需 | 默认 | 说明 |
|------|------|------|------|------|
| `annotations.priority` | float `[0, 1]` | 否 | `0.0` | **MCP 标准**；布局排序优先级；**仅在同一 MCP Server 内部窗口间比较**；值越大越靠前 |
| `annotations.audience` | `["assistant"]` / `["user"]` / `["user","assistant"]` | 推荐 | — | **MCP 标准**；Window 资源面向 Agent 渲染，**SHOULD** 显式声明 `["assistant"]`；声明 `["user"]` 时 v0.2 仅记录 WARN 不过滤（v0.3+ 可能升级为硬过滤） |
| `annotations.lastModified` | ISO 8601 字符串 | 否 | — | MCP 标准：资源最后修改时间；A2C v0.2 透传不用于组织排序 |

### A2C 扩展字段（`_meta`）

| 字段 | 类型 | 必需 | 默认 | 说明 |
|------|------|------|------|------|
| `_meta.fullscreen` | bool | 否 | `false` | **A2C 扩展**（无对应 MCP 标准字段）；全屏渲染标记；同一 Server 内仅首个 `fullscreen=true` 的窗口生效，其他窗口被排除 |

### 校验规则

1. `annotations.priority` 若存在，必须为 `[0.0, 1.0]` 范围内的浮点数；越界时 Computer 记录警告并视为 `0.0`
2. `_meta.fullscreen` 若存在，必须为布尔值；其他类型视为 `false` 并记录警告
3. `annotations.audience` 若声明 `["user"]`（即不含 `"assistant"`），Computer **MUST** 记录 WARN 但仍纳入 Desktop 聚合；v0.3+ 可能升级为硬过滤
4. `annotations` 字段若存在，必须符合 MCP 规范（见 [MCP Resource annotations](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#annotations)）
5. **缺失值同义**：`annotations is None` 与 `annotations.priority is None` 等价，统一按 `priority=0.0` 处理；`_meta is None` 与 `_meta.fullscreen is None` 等价，统一按 `fullscreen=false` 处理。Computer 不区分两种缺失原因（DEBUG 级日志即可，避免噪声）
...
Resource(
    ...
    annotations=Annotations(
        priority=0.8,                         # MCP 标准：布局排序（[0.0, 1.0]）
        audience=["assistant"],               # MCP 标准：Window 面向 Agent，SHOULD ["assistant"]
        lastModified="2026-01-15T08:30:00Z",  # MCP 标准：透传，不参与排序
    ),
    _meta={
        "fullscreen": True,  # A2C 扩展：全屏渲染（无对应 MCP 标准字段）
    },
)
```
...
Computer 在响应 `client:get_desktop` 时执行 **desktop 组织职能**——对原始窗口资源进行过滤、按 audience / priority 排序、按 size 截断，输出经过组织的 Desktop 列表。

> **协议视角**：A2C-SMCP 协议规定 desktop 组织的**行为契约**（过滤规则、排序规则、size 语义、错误情形），但**不规定** Computer SDK 内部用什么函数签名 / 参数形式实现。SDK 可以选择 `organize_desktop(windows, size, history)` 或 `(server_name, resource_list, size, history)` 或任何其他形式——只要**外部行为符合本节规定的契约**。具体 SDK 实现细节见各 SDK 文档。
...
同一 Server 内的窗口按 **`Resource.annotations.priority`** 降序排列（高优先级在前）。未声明、为 `None` 或越界时默认为 `0.0`。

!!! important "priority 来源"

    Computer **从 `Resource.annotations.priority`（MCP 标准字段，`float [0.0, 1.0]`）读取优先级**，不再解析 URI 中的 `?priority=` query（自 v0.2 起已移除）。`annotations is None` 与 `annotations.priority is None` 等价处理为 `0.0`。
...
在 `resources/list` 响应中返回 `window://` URI 的 Resource，并通过 `annotations` 声明 MCP 标准字段、`_meta` 声明 A2C 扩展字段：
...
async def list_resources():
    return [
        Resource(
            ...
            mimeType="text/plain",
            annotations=Annotations(
                priority=0.6,             # 可选：布局排序（[0.0, 1.0]）
                audience=["assistant"],   # 推荐：Window 面向 Agent
            ),
            _meta={
                "fullscreen": False,      # 可选：是否全屏（A2C 扩展）
            },
        ),
        Resource(
            ...
            description="实时控制台日志",
            mimeType="text/plain",
            annotations=Annotations(priority=0.3, audience=["assistant"]),
        ),
    ]
```

每个 Resource 的 `uri` 必须是有效的 `window://` URI（参见 [URI 格式](#uri-格式)）。字段语义见 [Window 资源元数据](#window-资源元数据)。
...
1. **host 使用反向域名风格**（如 `com.example.myapp`）以提升可读性 + 天然回避跨 Server 冲突；跨 MCP Server **SHOULD** 唯一（与 `dpe://` 共享 host 命名空间），冲突时 Computer 注册阶段记 WARN——详见 [DPE host 路由策略](dpe.md#host-路由策略)
2. **通过 `Resource.annotations.priority` 合理分配优先级**（[0.0, 1.0]，越大越靠前），避免所有窗口使用默认 `0.0`
3. **`Resource._meta.fullscreen` 仅用于需要完整展示的主窗口**，如当前浏览器页面或全屏编辑器
4. **`Resource.annotations.audience` SHOULD 显式声明 `["assistant"]`**：Window 面向 Agent 渲染；声明 `["user"]` 在 v0.2 仅触发 WARN 但不过滤
5. **不要在 URI 中附加 query 参数**：所有元数据通过 `annotations` / `_meta` 声明；URI 只表示身份
6. **窗口内容保持简洁、文本化**，便于 LLM 消费；避免嵌入大量原始 HTML 或二进制数据
7. **及时发送变更通知**，确保 Agent 获取到最新桌面状态
