...
1. `scheme` 必须为 `window`
2. `host` 不能为空
3. `host` 跨 MCP Server **SHOULD** 唯一（与 `dpe://` 共享 host 命名空间约束）。`organize_desktop` 本身按 MCP Server 名称分组、不依赖 host 唯一；但 `client:get_dpe` 通过 host 反查 MCP Server，host 重复会导致 dpe 资源被错误路由。Computer 在 MCP Server 注册时检测 host 冲突 + 记 WARN（不阻塞注册）；详见 [DPE host 路由策略](dpe.md#host-路由策略)。**实践建议**：采用反向域名风格（如 `com.example.editor`）天然回避冲突
