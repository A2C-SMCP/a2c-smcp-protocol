...
| `host` | 是 | MCP Server 唯一标识 | 不能为空；**在同一 Computer 进程范围内 MUST 全局唯一**（跨 Office/Room 也唯一，房间隔离不消歧 host）；推荐反向域名风格，如 `com.example.mcp` |
...
1. `scheme` 必须为 `window`
2. `host` 不能为空
3. `host` 在**同一 Computer 进程范围内 MUST 全局唯一**；当两个 MCP Server 声明相同 host 时，Computer **MUST** 报错并拒绝后注册的 Server 参与 Desktop 聚合（仅保留先注册的一方）。该唯一性约束跨 Office / Room 生效——不同 Computer 实例（即使在同一 Room）允许 host 重复，房间隔离层不参与 host 消歧。**作用域生命周期**：（a）host 索引仅存活于 **Computer 进程生命周期内**，进程重启后清空重建；（b）Computer 切换 office（先离开后加入另一 office）时 **不重置** host 索引——MCP Server 注册关系与 office 解耦，host 唯一性贯穿整个进程生命周期；（c）当 MCP Server 主动断开（如 Computer 卸载某个 MCP Server）时，host 占用解除，可被新注册的 Server 复用
...
1. **host 使用反向域名风格**（如 `com.example.myapp`）并**确保在同一 Computer 进程范围内全局唯一**（跨 Office/Room 也唯一；协议层约束，违反会导致该 Server 被 Computer 拒绝参与 Desktop 聚合）
