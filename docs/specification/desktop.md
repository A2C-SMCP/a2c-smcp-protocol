...
1. `scheme` 必须为 `window`
2. `host` 不能为空
3. `host` 跨 MCP Server **SHOULD** 唯一。Window URI 内部的 desktop 组织算法按 MCP Server 名称分组、不强依赖 host 唯一；冲突时 Computer 在 MCP Server 注册阶段记 WARN（lint-style 引导，不阻塞注册）。host **SHOULD** 使用反向域名风格（如 `com.example.editor`）
