...
> **授权失败的硬约束**（详见 [`error-handling.md` → MCP 上游授权错误响应](error-handling.md#mcp-上游授权错误响应)）：Computer 一旦判定上游失败**属授权类**——含 MCP 客户端库把授权失败吸收为语义错误变体而丢状态码的情形（如 rmcp `AuthRequired`）——**MUST** 映射为 `4006`/`4007`、**MUST NOT** 降级为 `4003` 亦不得返回缺 `meta.error_code` 的通用失败结果（[降级语义](error-handling.md#降级语义授权类失败的硬映射)）；且 **MUST NOT** 表现为调用挂起至 Agent 超时（[可观测判据](error-handling.md#可观测判据禁止挂起至超时)）。检测点归 SDK 自治，但"是否产出 4006/4007"与"是否挂起"是对端可感的硬约束。真实传输 conformance 向量见 [`fixtures/auth_error_conformance_vectors.json`](fixtures/auth_error_conformance_vectors.json)。
