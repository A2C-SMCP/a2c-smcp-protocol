...
Skill 引用对象——`client:get_skills` 返回列表的元素。`name` 是协议主键（合成的全局唯一名，跨工具对齐裸名：marketplace `<plugin>:<skill>` / user 裸 `<skill>` / mcp `mcp:<bundle_id>:<skill>`，见 [skill.md §1](skill.md#1-skill-命名)）；Agent 把 `name` 当**不透明可比较字符串**，判定来源用 `source` 字段。mcp 形态的 server 段取 [`bundle_id`](#bundleid)（server 唯一身份），非 display `name`。
...
class A2CSkillRef(TypedDict):       # 默认 total=True：裸字段 = 必选，NotRequired[] = 可选
    ...
    name: str                       # 必选：合成全局唯一名（跨工具对齐裸名）
                                    ...
                                    # mcp: mcp:<bundle_id>:<skill>，如 mcp:tfrobot-tools:code-review
    # ── 来源元数据 ───────────────────────────────────────
    source: str                     # 必选：完整来源 provenance（含 marketplace 名等）
                                    # 例：mcp:<bundle_id>（如 mcp:tfrobot-tools）/ marketplace:acme-skills / user
    ...
    tags: NotRequired[list[str]]            # frontmatter "tags" 透传（分类元数据）
                                            # 纯透传不校验：非 list[str] → 省略该字段（SKILL 照常注册，见 skill.md §1.5）
...
!!! note "`path` 恒存在；为何无 raw `mcp_server` 字段"
    ...
    **不暴露 raw display server 名（身份由 bundle_id 承载）**：理念 #2 要求 Agent 侧协议表面与 source 无关。MCP Server 的 raw display `name` 是纯人类可读、允许碰撞、**非身份**（见 [§身份正交性](#identity-orthogonality)），不进 SKILL ref。SKILL 的 server 身份统一由 **`bundle_id`** 承载——它是 `client:get_config` 的 `servers` **key**（旧版曾为 raw 名，post-BundleID 已改），并构成 `name`（`mcp:<bundle_id>:<skill>`）与 `source`（`mcp:<bundle_id>`）的 server 部分。来源追溯由 `source`（含 `bundle_id`）与 MCP 来源的 `uri`（skill.md §2）承担。

!!! note "`version` 的 source-of-truth（不来自 frontmatter）"

    marketplace SKILL v1 frontmatter 恰好 7 字段（`name` / `description` / `license` / `compatibility` / `tags` / `metadata` / `allowed-tools`），**无 version**。`A2CSkillRef.version` 按来源取值，故为 `NotRequired`（无来源即省略）：
...
| **SKILL name 的 `<server>` 段**（`mcp:<server>:<skill>`）| **= `bundle_id` 原样**（[skill.md §1](skill.md#1-skill-命名) §1.3）| SKILL mcp 形态 name / `source` 的 server 部分；唯一性由 `bundle_id` 的 [no-double-open](#no-double-open) 保证 | **是——即 `bundle_id`** |

- **`window://` / `skill://` 的 `host` 仍与 `bundle_id` 正交**：host 由 MCP Server 自选、A2C 透传不解释；一个 A2C server（`bundle_id`）内可含多个 MCP-自选 host 的资源。
- **SKILL name 的 `<server>` 段与 `bundle_id` 有意耦合**（`<server>` = `bundle_id`）：使 SKILL 的 server 身份与 `get_config.servers` key / `get_resources.mcp_server` / 错误码 `meta.mcp_server` 全协议统一；取 `bundle_id` 的唯一性令 mcp 形态 name **构造上不碰撞**（取 display `name` 则会因其可碰撞而牺牲合法 SKILL 可见性，见 [skill.md §1.5](skill.md#15-校验失败处理)）。
- 因 `get_config.servers` 的 key 现为 `bundle_id`，`name` 的旧隐式唯一性（曾作 dict key）**随之撤回**——`name` 可碰撞，仅供展示；故凡以 server 为身份的场合（SKILL `<server>` 段亦然）**MUST** 用 `bundle_id`。
