...
plugin 声明依赖的 MCP Server 以 `bundle_id` 与运行期权威配置集比对（[runtime-contract §2.5](runtime-contract.md)）：同 `bundle_id` 已存在 = 依赖已满足，MUST NOT 拒绝，按来源优先序 reconcile；display name 相同、`bundle_id` 不同是合法共存。无论何种情形，最终 Agent-facing 投影 MUST NOT 暴露歧义路由（`bundle_id` 键 + `{bundle_id}__` 工具前缀在构造上保证）。
...
6. 写入通道落盘目标属受信配置：settings 的 `landingRoot` 仅 trusted / policy scope 可设；**project scope 提供该键 MUST 被 Computer 拒绝**（防 clone 仓库把 Agent 写目标重定向到任意路径）。`client:put_blob` 落盘是协议事件明确定义的本地状态影响（同不变量 #2 的允许形态），其落点由本不变量约束。
...
| Plugin lifecycle | `a2c_smcp/computer/settings/installer.py` | `crates/smcp-computer/src/settings/installer.rs` | install/enable/disable/uninstall 概念和 MCP 依赖预检（bundle_id、依赖已满足不拒绝）共享 |
