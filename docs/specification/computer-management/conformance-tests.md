...
十类向量（#1–#8 在 reconcile 后 `SMCPTool.meta` 出线断言；wire 字符串化是既有既定行为，key 顺序不参与判定）：

| # | 配置 | 期望终值（`meta["a2c_tool_meta"]` 解析后） |
|---|---|---|
| 1 | 无任何 `tool_meta` / `default_tool_meta` | `tags=["read"]`、`auto_apply=null`（声明中白名单外字段被字段级过滤）；`custom_key` 原样保留 |
...
| 6 | 声明含 `auto_apply=true`（任意配置） | 终值 `auto_apply` 恒来自配置（无配置 → `null`），Server 自声明的值**不进入终值** |
| 7 | 声明畸形：`tags` 为字符串 / 声明非对象（白名单外字段过滤由 #1/#6/#8 覆盖） | `tools/list` 正常、工具可用；声明丢弃 + 诊断；有配置终值 → 维持配置 canonical，无配置终值 → `a2c_tool_meta` 键**删除** |
| 8 | 声明合法 + 配置覆盖 | 原生 `_meta` key（`custom_key`）原样保留；原 `a2c_tool_meta` 被 canonical **整体覆写**（不字段级残留） |
| 9 | 上述各输入 | Python / Rust 对同一输入，reconcile 后 `SMCPTool.meta` 逐字段一致 |
| 10 | `default_tool_meta.tags=["default"]` + `tool_meta["safe_read"].tags=null` | `tags=["default"]`（显式 null 与缺失等价，继承下一层） |

期望补充：

- Server 声明的值在任何配置下都不进入 canonical 终值——`auto_apply` 等白名单外字段的字段值被过滤（字段本身仍按配置出线，无配置时为 `null`）；
...
- **PickString 取值向量**（[runtime-contract §5.12](runtime-contract.md)）七景 MUST 双端对拍：① 已存值匹配任一 option.value → 渲染成功，注入值为 `value` 而非 `label`；② 已存值不匹配任一 option.value → 产出结构化 `invalid_selection`（MUST 携带 `id` 字段），**MUST NOT** 回退 default 或首项；③ 无用户值 → 先按解析链尝试 default；仍无值时 MAY 选首项（**不反向持久化**——下次解析仍从解析链开始）；④ default 不匹配任一 option.value → 配置校验拒绝（`validation`）；default 显式 `null` → **不**拒绝，视为无默认走解析链；⑤ 空 `options` / 空 `label` / 空 `value` → 配置校验拒绝；⑥ `label` / `value` 重复 → 校验通过（不要求唯一），且 MUST NOT 按 `value` 反推原选 `label`；⑦ 旧 `"options": ["a", "b"]` 字符串数组形式 → MUST 以 `validation` 拒绝（与结构化形式同一错误类别，双端一致，无 alias）。
- **Input 重解析时机向量**（[runtime-contract §5.13](runtime-contract.md)）六景 MUST 双端对拍：① stopped → 实际 start → resolver 最新值生效；② 实际 restart → resolver 最新值生效（改选后重启不再静默沿用旧渲染品）；③ 运行中修改 resolver 返回值 → 运行实例不热更新，值保持；④ 已运行 server 幂等 start MAY no-op（不重复解析，同一次解析结果复用）；⑤ 单次 server 渲染中相同 id 只解析一次；⑥ 实际启动重解析失败 → 结构化错误，且尽量保留仍在运行的旧进程（若有）。
