...
**路由说明**：DPE URI 是**自包含寻址凭据**——Computer 通过 URI 中的 `host` 反查目标 MCP Server。host 跨 Server SHOULD 唯一（非 MUST）；冲突时 Computer 在注册阶段记 WARN、路由按"先注册优先"。详见 [DPE 文档协议 - host 路由策略](dpe.md#host-路由策略)。
