...
| `OPEN_DPE_EVENT` | `client:open_dpe` | 打开 DPE 文档（解析为访问 URI）| `OpenDPEReq` | `OpenDPERet` |
...
**错误码**：未注册 Resolver → `4011 DPE Resolver Not Configured`；URI 非法 → `4012 Invalid DPE URI`；Resolver 执行失败 → `4013 DPE Resolution Failed`。
...
sequenceDiagram
    ...
    A->>S: client:open_dpe(dpe://host/doc-1)
    S->>C: 路由到目标 Computer
