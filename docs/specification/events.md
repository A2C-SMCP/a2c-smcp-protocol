...
把一个 DPE URI 转成 Agent 可访问的 URI。Computer 调本地注册的 [DPE Resolver hook](dpe.md#dpe-resolver-hook业务层) 完成转换；Resolver 业务自决（上传对象存储 / 落本地缓存 / 任意 URI scheme）。Agent 拿到访问 URI 后用应用层协议（HTTP / file / ...）自取实际内容——A2C 协议**不**承载 DPE 内容。
...
**路由说明**：DPE URI 是**自包含寻址凭据**——Computer 通过 URI 中的 `host` 反查目标 MCP Server。host 跨 Server **MUST** 唯一——Computer 在 MCP Server 注册阶段强制检测，冲突时**注册失败**（硬约束）。运行时不存在路由歧义。详见 [DPE 文档协议 - host 路由策略](dpe.md#host-路由策略)。

**响应数据 (GetDPERet)**: `GetDPERet = A2CResource + req_id`——Resolver 输出的 [A2CResource](data-structures.md#a2cresource) 加上协议级 `req_id` 字段。访问层 hint（过期、内容指纹）通过 `_meta.expires_at` / `_meta.etag` 承载。

```python
{
    "uri": str,                       # 必选：Resolver 输出的访问 URI（任意 scheme）
    "mime_type": str,                  # 可选
    "size": int,                       # 可选：字节数
    "name": str,                       # 可选
    "description": str,                # 可选
    "annotations": dict,               # 可选
    "_meta": {                         # 可选
        "expires_at": str,             # ISO 8601；URI 过期时间
        "etag": str                    # 内容指纹
    },
    "req_id": str
}
...
| 码 | 触发 |
|----|------|
| `4011 DPE Resolver Not Configured` | 未注册 Resolver |
| `4012 Invalid DPE URI` | URI 不符合 dpe scheme（构造层硬错误）|
| `4013 DPE Resolution Failed` | Resolver / 上游 / mimetype 失败——`category` 字段细分（`upstream_unavailable` / `invalid_dpe_mime` / `resolver_error` / `resolver_returned_invalid`）|
| `4014 MCP Server Not Found` | URI 中的 host 反查不到对应 MCP Server |
| `4015 MCP Capability Not Supported` | MCP Server 已注册但未声明 `resources` capability |

详见 [DPE 文档协议](dpe.md) 与 [错误处理 §4013](error-handling.md#dpe-解析失败4013)。
