...
!!! note "`SMCPTool.name` 是 exposed_tool_name；归属看 `bundle_id` 字段"
    ...
    每个工具的 **server 归属由显式 `SMCPTool.bundle_id` 字段承载**（值 = 所属 server 的解析后 bundle_id，与 [`client:get_config`](#clientget_config) 响应 `servers` 的 key 同一身份空间）；Agent **MUST NOT** 切分 `name` 的 `__` 前缀反推归属。见 [SMCPTool](data-structures.md#smcptool)。
